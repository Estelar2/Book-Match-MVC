...
<!DOCTYPE html><html lang="pt-br"><head><meta charset="UTF-8"><title>Cadastrar Livro</title>
<style>
...
<body>
<div class="nav"><div class="brand">📚 BookMatch MVC</div><div><a href="index.jsp">Início</a><a href="listar.jsp">Biblioteca</a><a href="recomendacoes.jsp">Recomendações</a></div></div>
<main class="container"><section class="panel"><div class="top"><h1>Cadastrar Livro</h1><a class="btn gray" href="index.jsp">Voltar</a></div>
<form class="form" action="livro" method="post"><input type="hidden" name="acao" value="cadastrar">
<div><label>Título</label><input type="text" name="titulo" placeholder="Ex: O Hobbit" required></div>
<div><label>Autor</label><input type="text" name="autor" placeholder="Ex: J.R.R. Tolkien" required></div>
<div><label>Gênero</label><select name="genero" required><option>Fantasia</option><option>Romance</option><option>Suspense</option><option>Ficção Científica</option><option>Terror</option><option>Aventura</option><option>Biografia</option><option>Drama</option></select></div>
<div><label>Avaliação</label><input type="number" name="avaliacao" min="0" max="10" step="0.1" placeholder="0 a 10" required></div>
<div><label>Descrição</label><textarea name="descricao" placeholder="Escreva uma breve descrição do livro..." required></textarea></div>
<div class="actions"><button type="submit">Salvar Livro</button><a class="btn gray" href="listar.jsp">Ver Biblioteca</a></div>
</form></section></main>
</body></html>

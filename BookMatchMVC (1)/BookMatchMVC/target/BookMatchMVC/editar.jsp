...
<%@ page import="model.Livro" %><%@ page import="service.LivroService" %>
<% int id = Integer.parseInt(request.getParameter("id")); LivroService service = new LivroService(); Livro livro = service.buscarPorId(id); if (livro == null) { response.sendRedirect("listar.jsp"); return; } %>
<!DOCTYPE html><html lang="pt-br"><head><meta charset="UTF-8"><title>Editar Livro</title>
<style>
...
<body>
<div class="nav"><div class="brand">📚 BookMatch MVC</div><div><a href="index.jsp">Início</a><a href="listar.jsp">Biblioteca</a></div></div>
<main class="container"><section class="panel"><div class="top"><h1>Editar Livro</h1><a class="btn gray" href="listar.jsp">Voltar</a></div>
<form class="form" action="livro" method="post"><input type="hidden" name="acao" value="atualizar"><input type="hidden" name="id" value="<%= livro.getId() %>">
<div><label>Título</label><input type="text" name="titulo" value="<%= livro.getTitulo() %>" required></div>
<div><label>Autor</label><input type="text" name="autor" value="<%= livro.getAutor() %>" required></div>
<div><label>Gênero</label><select name="genero" required><option value="<%= livro.getGenero() %>"><%= livro.getGenero() %></option><option>Fantasia</option><option>Romance</option><option>Suspense</option><option>Ficção Científica</option><option>Terror</option><option>Aventura</option><option>Biografia</option><option>Drama</option></select></div>
<div><label>Avaliação</label><input type="number" name="avaliacao" min="0" max="10" step="0.1" value="<%= livro.getAvaliacao() %>" required></div>
<div><label>Descrição</label><textarea name="descricao" required><%= livro.getDescricao() %></textarea></div>
<div class="actions"><button type="submit">Atualizar Livro</button></div>
</form></section></main>
</body></html>

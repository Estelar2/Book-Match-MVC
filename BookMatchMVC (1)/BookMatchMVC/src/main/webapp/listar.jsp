...
<% LivroService service = new LivroService(); List<Livro> livros = service.listar(); String msg = request.getParameter("msg"); %>
<!DOCTYPE html><html lang="pt-br"><head><meta charset="UTF-8"><title>Biblioteca</title>
<style>
...
<body>
<div class="nav"><div class="brand">📚 BookMatch MVC</div><div><a href="index.jsp">Início</a><a href="cadastrar.jsp">Cadastrar</a><a href="recomendacoes.jsp">Recomendações</a></div></div>
<main class="container"><div class="top"><h1>Biblioteca</h1><a class="btn success" href="cadastrar.jsp">+ Novo Livro</a></div>
<% if ("cadastrado".equals(msg)) { %><div class="alert">Livro cadastrado com sucesso!</div><% } else if ("atualizado".equals(msg)) { %><div class="alert">Livro atualizado com sucesso!</div><% } else if ("excluido".equals(msg)) { %><div class="alert">Livro excluído com sucesso!</div><% } %>
<div class="grid"><% for (Livro livro : livros) { %><div class="card"><h3><%= livro.getTitulo() %></h3><div class="muted"><%= livro.getAutor() %></div><span class="badge"><%= livro.getGenero() %></span><p class="desc"><%= livro.getDescricao() %></p><div class="rating">⭐ <%= livro.getAvaliacao() %>/10</div><div class="actions"><a class="btn warning" href="editar.jsp?id=<%= livro.getId() %>">Editar</a><a class="btn danger" href="livro?acao=excluir&id=<%= livro.getId() %>" onclick="return confirm('Deseja excluir este livro?')">Excluir</a></div></div><% } %></div>
</main>
</body></html>

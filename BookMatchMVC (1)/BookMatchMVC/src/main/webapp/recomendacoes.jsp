...
<%@ page import="java.util.List" %><%@ page import="model.Livro" %><%@ page import="service.LivroService" %>
<% String genero = request.getParameter("genero"); LivroService service = new LivroService(); List<Livro> livros = null; if (genero != null && !genero.trim().isEmpty()) { livros = service.recomendarPorGenero(genero); } %>
<!DOCTYPE html><html lang="pt-br"><head><meta charset="UTF-8"><title>Recomendações</title>
<style>
...
<body>
<div class="nav"><div class="brand">📚 BookMatch MVC</div><div><a href="index.jsp">Início</a><a href="listar.jsp">Biblioteca</a><a href="cadastrar.jsp">Cadastrar</a></div></div>
<main class="container"><section class="hero"><h2>Recomendações por Gênero</h2><p>Escolha um gênero literário e veja sugestões ordenadas pelas melhores avaliações.</p>
<form class="actions" method="get" action="recomendacoes.jsp"><select name="genero" required><option value="">Selecione um gênero</option><option>Fantasia</option><option>Romance</option><option>Suspense</option><option>Ficção Científica</option><option>Terror</option><option>Aventura</option><option>Biografia</option><option>Drama</option></select><button type="submit">Buscar</button></form></section>
<% if (livros != null) { %><div class="grid"><% if (livros.isEmpty()) { %><div class="empty">Nenhum livro encontrado para este gênero.</div><% } else { for (Livro livro : livros) { %><div class="card"><h3><%= livro.getTitulo() %></h3><div class="muted"><%= livro.getAutor() %></div><span class="badge"><%= livro.getGenero() %></span><p class="desc"><%= livro.getDescricao() %></p><div class="rating">⭐ <%= livro.getAvaliacao() %>/10</div></div><% } } %></div><% } %>
</main>
</body></html>
